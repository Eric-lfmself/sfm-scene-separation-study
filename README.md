...
identical image pairs, an identical feature budget, and an identical mapper.

I expected to be documenting how the learned front end fails in one specific way. What I
measured is that on this data it does not beat the classical one at all, and that the
failure I set out to document is caused by the learned matcher rather than being a
...
| `colmap-shortlist` | DINOv2 shortlist | SIFT | nearest neighbour | 4600 features, 1024 px |
| `colmap-default` | exhaustive | SIFT | nearest neighbour | stock: 8192 features, 3200 px |

`learned` and `colmap-shortlist` see **the same image pairs at the same feature budget and
the same input resolution**; the only difference is the detector and the matcher.
...
| Scene | learned | SIFT @ matched budget | SIFT @ stock |
|---|---|---|---|
...
| SIFT @ matched budget | 69/75 | 3 ✓ | 1.000 | **1.000** (559/559) |
...
**This is not about feature density.** The stock configuration extracts 11928 keypoints
per image — 2.6× the learned budget, at three times the resolution — and produced **zero**
cross-scene verified pairs. More features did not manufacture false links; the learned
matcher did.
...
| SIFT @ matched budget | **1** ✓ | **0.674**, 100% | **0.899 · 8 mm · 0.337°** |
...
right in both — at 1/2.6 of the feature budget and 1/6 of the input resolution.
...
- I ran the learned front end at 1024 px. The matched-budget SIFT arm was handicapped the
  same way, so the comparison between them is fair, but both are below what a production
  pipeline would use — and the `pipes` row shows how much resolution can matter.
- The aerial comparison on Mill 19 is in progress and not yet included.
